...
if [[ -z $ENABLE_PROMETHEUS ]]; then
  echo "Reporting results to a file in `results` folder"
  ...
  podman run --rm -v $PWD:/code -p 6565:6565 -i grafana/k6 -e K6_PROMETHEUS_RW_SERVER_URL=http://$PROMETHEUS_IP:9090/api/v1/write -e K6_PROMETHEUS_RW_TREND_AS_NATIVE_HISTOGRAM=true -e K6_OUT=experimental-prometheus-rw -e DOMAIN=$DOMAIN -e DATE=$date -e BASE_REQUEST_TARGET=$BASE_REQUEST_TARGET -e SERVICE_COUNT=$SERVICE_COUNT run /code/tests/tests.js --tag testid=$date > results/"${date}"-k6-stats.log 2>&1
fi
